...
\subsection{The weakening rule in \texorpdfstring{$\lambda\omega$}{}}\label{sub:weak:omega}
...
\AxiomC{$A$}
\AxiomC{$A \implies \perp$}
\LeftLabel{\textit{($\perp$-intro)}\quad}
\BinaryInfC{$\perp$}
\DisplayProof
...
\begin{itemize}
    \item $\Pi C : * . (A \to B \to C) \to C$: For all $C$, ($A$ implies ($B$ implies $C$)) implies $C$
    \item Or \textit{if $A$ and $B$ together imply $C$, $C$ holds}
\end{itemize}



Introduction and elimination rule for \textit{second order encodings} compared with Propositional logic:

\begin{tabular}{c c}
    \AxiomC{$A$}
    \AxiomC{$B$}
    \LeftLabel{\textit{($\land$-intro)}\quad}
    \BinaryInfC{$A \land B$}
    \DisplayProof 
    & \quad
    \AxiomC{$A$}
    \AxiomC{$B$}
    \LeftLabel{\textit{($\land$-intro-sec)}\quad}
    \BinaryInfC{$\Pi C : * (A \to B \to C) \to C$}
    \DisplayProof 
    \\ & \\
    \AxiomC{$A \land B$}
    \LeftLabel{\textit{($\land$-elim-left)}\quad}
    \UnaryInfC{$A$}
    \DisplayProof 
    & \quad
    \AxiomC{$\Pi C : * (A \to B \to C) \to C$}
    \LeftLabel{\textit{($\land$-elim-left-sec)}\quad}
    \UnaryInfC{$A$}
    \DisplayProof 
    \\ & \\
    \AxiomC{$A \land B$}
    \LeftLabel{\textit{($\land$-elim-right)}\quad}
    \UnaryInfC{$B$}
    \DisplayProof 
    & \quad
    \AxiomC{$\Pi C : * (A \to B \to C) \to C$}
    \LeftLabel{\textit{($\land$-elim-right-sec)}\quad}
    \UnaryInfC{$B$}
    \DisplayProof 
\end{tabular}
...
\AxiomC{$\Gamma \deriv a : A$}
\AxiomC{$\Gamma \deriv b : B$}
\LeftLabel{\textit{($\land$-intro-sec-tt)}\quad}
\BinaryInfC{$\Gamma \deriv ?_1 : \Pi C : *. (A \to B \to C) \to C$}
\DisplayProof 

\AxiomC{$\Gamma \deriv c : \Pi C : *. (A \to B \to C) \to C$}
\LeftLabel{\textit{($\land$-elim-left-sec-tt)}\quad}
\UnaryInfC{$\Gamma \deriv ?_2 : A$}
\DisplayProof 

\AxiomC{$\Gamma \deriv c : \Pi C : *. (A \to B \to C) \to C$}
\LeftLabel{\textit{($\land$-elim-right-sec-tt)}\quad}
\UnaryInfC{$\Gamma \deriv ?_3 : B$}
\DisplayProof 

In flag derivation \textit{($\land$-intro-sec)} format to find $?_1, ?_2, ?_3$ we have:
...
\begin{flagderiv}
    ...
    \assume{a:16:4}{y : B}{}
    \assume{a:16:5}{C : *}{}
    \assume{a:16:6}{z : A \to B \to C}{}
    \step{s:16:1}{zx : B \to C}{(appl) on \ref{a:16:6} and \ref{a:16:3}}
    \step{s:16:2}{zxy : C}{(appl) on \ref{s:16:1} and \ref{a:16:4}}
    \conclude{c:16:1}{\lambda z : A \to B \to C . zxy : (A \to B \to C) \to C}{(abst) on \ref{s:16:2}}
    \conclude{c:16:2}{\lambda C : *. \lambda z : A \to B \to C . zxy : \\ \Pi C : *. (A \to B \to C) \to C}{(abst) on \ref{c:16:1}}
\end{flagderiv}

\textbf{\textit{Disjunction}}

$A \lor B \equiv \Pi C : * . (A \to B) \to (B \to C) \to C$, which is:

\begin{itemize}
    \item $\Pi C : * . (A \to B) \to (B \to C) \to C$: For all $C$, ($A \to C$ implies that ($B \to C$ implies $C$)
    \item Or \textit{if $A$ implies $C$ and also $B$ implies $C$, $C$ holds on its own}
\end{itemize}

In natural deduction rules:

\AxiomC{$A$}
\LeftLabel{\textit{($\lor$-intro-left)}\quad}
\UnaryInfC{$A \lor B$}
\DisplayProof 

\AxiomC{$B$}
\LeftLabel{\textit{($\lor$-intro-right)}\quad}
\UnaryInfC{$A \lor B$}
\DisplayProof 
...
\AxiomC{$A \implies C$}
\AxiomC{$B \implies C$}
\LeftLabel{\textit{($\forall$-elim)}\quad}
\TrinaryInfC{$C$}
\DisplayProof 
...
\AxiomC{$A$}
\LeftLabel{\textit{($\lor$-intro-left-sec)}\quad}
\UnaryInfC{$\Pi C : * . (A \to B) \to (B \to C) \to C$}
\DisplayProof 

\AxiomC{$B$}
\LeftLabel{\textit{($\lor$-intro-right-sec)}\quad}
\UnaryInfC{$\Pi C : * . (A \to B) \to (B \to C) \to C$}
\DisplayProof

\AxiomC{$\Pi D : * . (A \to D) \to (B \to D) \to D$}
\AxiomC{$A \to C$}
\AxiomC{$B \to C$}
\LeftLabel{\textit{($\forall$-elim-sec)}\quad}
\TrinaryInfC{$C$}
\DisplayProof 

In flag derivation \textit{($\lor$-intro-sec)} 

\begin{flagderiv}
    \assume{a:17:1}{A : *}{}
    \assume{a:17:2}{B : *}{}
    \assume{a:17:3}{C : *}{}
    \assume{a:17:4}{x : (\Pi D : *. (A \to D) \to (B \to D) \to D)}{}
    \assume{a:17:5}{y : A \to C)}{}
    \assume{a:17:6}{z : B \to C)}{}
    \step{s:17:1}{xC :  (A \to C) \to (B \to C) \to C}{(appl) on \ref{a:17:4} and \ref{a:17:3}}
    \step{s:17:2}{xCy : (B \to C) \to C}{(appl) on \ref{s:17:1} and \ref{a:17:5}}
    \step{s:17:3}{xCyz : C}{(appl) on \ref{s:17:2} and \ref{a:17:6}}
\end{flagderiv}
...
\begin{flagderiv}
    \assume{a:18:1}{A : *}{}
    \assume{a:18:2}{B : *}{}
    \assume{a:18:3}{x : (\Pi C : *. (A \to C) \to (B \to C) \to C)}{}
    \assume{a:18:4}{y : A \to \perp}{}
    \step{s:18:1}{xB :  (A \to B) \to (B \to B) \to B}{(appl) on \ref{a:18:2} and \ref{a:18:3}}
    \assume{a:18:5}{u : A}{}
    \step{s:18:2}{yu : \perp}{(appl) on \ref{a:18:5} and \ref{a:18:4}}
    \step{s:18:3}{yuB : B}{(appl) on \ref{s:18:2} and \ref{a:18:2}}
    \conclude{c:18:1}{\lambda u : A. yuB : A \to B}{(abst) on \ref{s:18:3}}
    \step{s:18:4}{xB(\lambda u : A. yuB) : (B \to B) \to B}{(appl) on \ref{s:18:1} and \ref{c:18:1}}
    \assume{a:18:6}{v : B}{}
    \step{s:18:5}{v : B}{(var) on \ref{a:18:6}}
    \conclude{c:18:2}{\lambda v : B. v : B \to B}{(abst) on \ref{s:18:5}}
    \step{s:18:6}{xB(\lambda u : A. yuB)(\lambda v : B. v):B}{(appl) on \ref{s:18:4} and \ref{c:18:2}}
    \conclude{c:18:3}{\lambda y : A \to \perp. xB(\lambda u : A. yuB)(\lambda v : B. v) : \\(A \to perp) \to B}{(abst) on \ref{s:18:6}}
    \conclude{}{\lambda x : (\Pi C : *. (A \to C) \to (B \to C) \to C). \\ \lambda y : A \to \perp. xB(\lambda u : A. yuB)(\lambda v : B. v) : \\ (\Pi C : *. ((A \to C) \to (B \to C) \to C)) \to (A \to \perp) \to B}{(abst) on \ref{c:18:3}}
...
\begin{flagderiv}
    \assume{}{i_{ET} : \Pi \alpha : *. \alpha \lor \neg \alpha}{}
    \assume{}{\beta : *}{}
    \assume{}{x: \neg\neg\beta}{}
    \step{}{i_{ET}\beta : \beta \lor \neg\beta}{}
    \step{}{i_{ET}\beta : \Pi \gamma : * . (\beta \to \gamma) \to (\neg\beta \to \gamma) \to \gamma}{}
    \step{}{i_{ET}\beta\beta : (\beta \to \beta) \to (\neg\beta \to \beta) \to \beta}{}
    \step{}{\lambda y : \beta . y : \beta \to \beta}{}
    \step{}{i_{ET}\beta\beta(\lambda y : \beta . y) : (\neg\beta \to \beta) \to \beta}{}
    \assume{}{z : \neg\beta}{}
    \step{}{xz : \perp}{}
    \step{}{xz\beta : \beta}{}
    \conclude{}{\lambda z : \neg\beta. xz\beta : \neg\beta \to \beta}{}
    \step{}{i_{ET}\beta\beta(\lambda y : \beta . y)(\lambda z : \neg\beta. xz\beta ) : \beta}{}
...
\AxiomC{$\forall_{x \in S}(P(x) \implies A)$}
\LeftLabel{\textit{($\exists$-elim)}\quad}
\BinaryInfC{$A$}
\DisplayProof 

In type theory:

\AxiomC{$\Pi \alpha : * . ((\Pi x : S. (Px \to \alpha) \to \alpha)$}
\AxiomC{$\Pi x : S. (Px \to A)$}
\LeftLabel{\textit{($\exists$-elim-sec)}\quad}
\BinaryInfC{$A$}
\DisplayProof 
...
\begin{flagderiv}
    ...
    \assume{}{P : S \to *}{}
    \assume{}{A : *}{}
    \assume{}{y : \Pi \alpha : * . ((\Pi x : S. (Px \to \alpha) \to \alpha)}{}
    \assume{}{z : \Pi x : S . (Px \to A)}{}
    \step{}{yA : (\Pi x : S . (Px \to A)) \to A}{}
    \step{}{yAz : A}{}
...
\AxiomC{$a \in S$}
\AxiomC{$P(a)$}
\LeftLabel{\textit{($\exists$-intro)}\quad}
...
\AxiomC{$a : S$}
\AxiomC{$P a$}
\LeftLabel{\textit{($\exists$-intro-sec)}\quad}
\BinaryInfC{$\Pi \alpha : * . ((\Pi x : S. (Px \to \alpha) \to \alpha)$}
\DisplayProof 
...
\begin{flagderiv}
    ...
    \assume{}{u : Pa}{}
    \step{}{\vdots}{}
    \step{}{? : \Pi \alpha : * . ((\Pi x : S. (Px \to \alpha) \to \alpha)}{}
...
\begin{flagderiv}
    ...
    \assume{a:20:4}{y : S}{}
    \assume{a:20:5}{v : Py}{}
    \step{s:20:1}{\lambda \alpha : *. \lambda w : (\Pi x : S . (Px \to \alpha)). wyv : \exists x : S.Px}{}
    \step{s:20:2}{u(\lambda \alpha : *. \lambda w : (\Pi x : S . (Px \to \alpha)). wyv) : \perp}{(appl) on \ref{a:20:3} and \ref{s:20:1}}
    \conclude{c:20:1}{\dots : \neg(Py)}{(abst) on \ref{s:20:2}}
    \conclude{c:20:2}{\dots : \forall y : S. \neg(Py)}{(abst) on \ref{c:20:1}}
    \conclude{c:20:3}{\dots : \neg (\exists x : S . Px) \implies \forall y : S . \neg P(y)}{(abst) on \ref{c:20:2}}
...
\begin{proof}[Derivation]
    ...
    \begin{flagderiv}
        \assume{}{L_{dn} : \Pi \gamma : *. \neg\neg\gamma \to \gamma}{}
        \assume{}{A : *}{}
        \assume{}{x: \Pi A : *. ((\neg \neg A \to A) \to (A \to A) \to A)}{}
        \assume{}{y : A}{}
        \step{}{L_{dn} A : \neg \neg A \to A}{}
        \step{}{x L_{dn} A : (A \to A) \to A}{}
        \conclude{}{\lambda y : A . y : A \to A}{}
        \step{}{x L_{dn} A y : A}{}
    \end{flagderiv}
\end{proof}
