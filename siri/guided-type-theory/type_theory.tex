...
\begin{definition}[Legal $\lambda\ra$-terms]
    A pre-typed term $M$ in $\lambda\ra$ is called \textbf{\textit{legal}} if there exist a context $\Gamma$ and type $\rho$ such that $\Gamma \vdash M : \rho$
\end{definition}
...
Having the following derivation:

\begin{prooftree}
    \AxiomC{$y : \alpha \ra \beta, z : \alpha \vdash y : \alpha \ra \beta$}
    \AxiomC{$y : \alpha \ra \beta, z : \alpha \vdash z : \alpha$}
    \BinaryInfC{$y : \alpha \ra \beta, z : \alpha \vdash yz : \beta$} 
    \UnaryInfC{$y : \alpha \ra \beta \vdash \lambda z : \alpha . yz : \alpha \ra \beta$} 
    \UnaryInfC{$\emptyset \vdash \lambda y : \alpha \ra \beta . \lambda z : \alpha . yz : (\alpha \ra \beta) \ra \alpha \ra \beta$} 
\end{prooftree}
