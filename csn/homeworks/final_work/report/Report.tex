...
In this work, we are going to formulate some hypothesis which we believe we can empirically prove, that are going to show the relationship between this principles and how to measure with \textbf{\acrfull{cnt}}.
At the same time, we are going to analysis different kinds of software of different sizes and build under different language paradigms to see if that tool set that \acrshort{cnt} provides are suitable for the general case. 
