...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
  ...
  \node[state]         (3MW)                  {$3MW$};
  \node[state]         (2MW) [left of=3MW]    {$2MW$};
  \node[state]         (F)   [left of=2MW]   {$F$};

  \path (3MW) edge [loop below] node {2/3} (3MW)
        (3MW) edge              node {1/3} (2MW)
        (2MW) edge [bend left]  node {3/4} (3MW)
        (2MW) edge [loop below] node {1/4} (2MW)
        (2MW) edge [bend left]  node {1/3} (F)
