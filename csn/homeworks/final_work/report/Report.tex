...
But if we want to be more rigorous we will perform a statistical test to comfirm it. Here the Null model is that this two variables are independent. So our alternative model that we propose is a linear model like this:

modularity = a + b * lines_of_code

So we perform an ANOVA test between this 2 models and the results are displayed in the following picture:
