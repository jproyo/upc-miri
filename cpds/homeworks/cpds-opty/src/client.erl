...
start(Location, ClientID, Entries, Reads, Writes, Server) ->
    spawn(Location, fun() -> open(ClientID, Entries, Reads, Writes, Server, 0, 0) end).
...
do_read(Entries, Handler) ->
    Ref = make_ref(),
    Pos = rand:uniform(length(Entries)),
    Num = lists:nth(Pos, Entries),
