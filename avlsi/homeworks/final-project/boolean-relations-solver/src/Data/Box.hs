...
instance Show Solution where
  show Solution {..} =
    P.show boxesC
      <> (toString $ unlines $ P.show lengthRoll : (P.show <$> propBoxes))
...
fromInput :: IO Boxes
fromInput = do
  [width, amountBoxes] <- safeCheck . lineToHeader <$> P.getLine
...
lineToHeader = traverse (first toString . readEither . toString) . words

lineBox :: Text -> Either String BoxInfo
lineBox = (toBox =<<) . traverse (first toString . readEither . toString) . words
