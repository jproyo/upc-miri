...
select (Just a) (Just b) = do
  ...
  return $
    ( orEmpty (and [validA, validB, aBetterB]) a
...
doWhile ::
  MonadIO m => (a -> m Bool) -> m a -> (a -> a -> m a) -> a -> m a
...
cnfDescription clausesList =
  let amountVars = U.last . nub . sort . map abs . concat $ clausesList
